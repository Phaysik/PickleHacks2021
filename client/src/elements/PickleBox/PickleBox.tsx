...
export default function PickleBox(props: PickleBoxAttrs) {
	...
	const changePickle = () => {
		if (props.currentPickle) props.currentPickle(props.name);
	};
	const capFirstLetter = (word: string) => {
		...
		["'", '-'].forEach((lowerSplitChar) => {
			if (word.includes(lowerSplitChar))
				char += word.split(lowerSplitChar)[1][0].toLowerCase();
		});
		return char;
	};
	const ignoreNonSymbolChars = (name: string) =>
		name.replace(/[^a-zA-Z\-'\s]/, ' ');
	const getSymbol = (name?: string) =>
		name ? ignoreNonSymbolChars(name).split(' ').map(capFirstLetter) : '??';
	const backgroundImage = `url(/images/pickle-${props.id}.jpg)`;

	return (
		<div
			className={classes}
			onMouseOver={changePickle}
